...
app_refs() {
  ...
  local apt_helpers="Alps-Images/common/apt-helpers.sh"
  ...
  if grep -Fq "$apt_helpers" "$dockerfile"; then
    [[ -f "$apt_helpers" ]] || { echo "ERROR: missing $apt_helpers" >&2; return 1; }
    hash_paths="$hash_paths $apt_helpers"
  fi
  local tag="${ALPS_REV}"
